...
# ╔═╡ 14ad4e70-32b3-11ed-223b-0504886c1561
begin
	using DifferentialEquations, Plots
	theme(:dracula)
end

# ╔═╡ d4b0fa92-6a0f-4ddb-b2e6-42f7583d7065
md"""
## Ejercicio 18
Considerar la ecuación del oscilador armónico amortiguado:
```math
...
1. Resolver numéricamente para $f=0$ tomando valores de $m$, $b$ y $k$ tales que $\Delta>0$, $\Delta<0$ y $\Delta=0$, siendo $\Delta = b^2-4mk$. Graficar y corroborar que el comportamiento obtenido es el esperado.
2. Para el caso subamortiguado, $\Delta<0$. Resolver numéricamente con $f(t,x) = \sin(t).$
3. Repetir con $f(t,x) = \cos(\lambda t)$ donde $\lambda = \frac{\sqrt{4mk-b^2}}{2m}$. ¿Qué se observa?
4. Resolver para $f=0$ y $b=0$, $k=m=1$ para tiempo entre $0$ y $1000$. ¿Qué se observa? ¿Es correcto este comportamiento? Una propiedad importante de este caso es que, al no haber rozamiento, se conserva la energía, siendo $E = \frac{1}{2}kx^2 + \frac{1}{2}m\dot{x}^2$. Para cada punto de la solución, calcular $E$ y decidir si se mantiene constante. Volver a resolver fijando valores chicos para ``abstol`` y ``reltol``. ¿Qué ocurre ahora? 
5. En muchos contextos es importante que el método numérico preserve este tipo de propiedades físicas. Más allá de la reducción de las tolerancias, existe la posibilidad de forzar al solver a mantener la solución en un determinado subconjunto del espacio (es decir: a cumplir una restricción). Para ello podemos resolver utilizando una callback function de la forma: 
```julia
    cb  = ManifoldProjection(g)
    sol = solve(prob,Tsit5(),save_everystep=false,callback=cb) 
```
donde ``g=g(res,u,p,t)`` es una función definida por el usuario en la que ``res`` es el output. Con esta sintaxis el método procura que la solución cumpla que ``res``$=0$ para todo punto. Implementar una función ``g`` adecuada, tal que cuando $g=0$ se tenga que $E$ es constante. Resolver.

"""

# ╔═╡ 5236ff73-5e07-4892-8833-607c5a6af9b9
function ejercicio18()
	...
	p2  = [1, 1, 1] # Δ < 0
	p3 = [1, 3, 1] # Δ > 0
	P1 = ODEProblem(oscilador, u₀, t, p1)
	P2 = ODEProblem(oscilador, u₀, t, p2)
	P3 = ODEProblem(oscilador, u₀, t, p3)
	sol1 = solve(P1)
	sol2 = solve(P2)
	sol3 = solve(P3)
	plot(sol1, idxs = 1, label = "Δ = 0")
	plot!(sol2, idxs = 1, label = "Δ < 0")
	plot!(sol3, idxs = 1, label = "Δ > 0")

	#b)
		function oscilador_sub(du, u, p, t)
			m, b, k = p
			du[1] = u[2]
			du[2] = (sin(t)-b*u[2]-k*u[1])/m
		end
		P4 = ODEProblem(oscilador_sub, u₀, t, [1, 0, 2])
		sol4 = solve(P4)
		plot!(sol4, idxs = 1, label = "Item b)")

	#c)
	function oscilador_3(du, u, p, t)
			m, b, k = p
			λ = ((4*m*k-b^2)^0.5)/2*m
			du[1] = u[2]
			du[2] = (cos(λ*t)-b*u[2]-k*u[1])/m
		end
		P5 = ODEProblem(oscilador_3, u₀, t, [1, 0, 2])
		sol5 = solve(P5)
		plot!(sol5, idxs = 1, label = "Item c)")
end

# ╔═╡ 4733a262-112e-4cb8-aa14-e26cbead63c0
ejercicio18()

# ╔═╡ 145fec4c-9cbf-4228-b043-bec97bd252bf
function ejercicio18_3()
	function oscilador(du, u, p, t)
		m, b, k = p
		du[1] = u[2]
		du[2] = (-b*u[2]-k*u[1])/m
	...
	P1 = ODEProblem(oscilador, u₀, t, p1)
	sol1 = solve(P1)
	plot(sol1, idxs = 1, label = "item 4")
end

# ╔═╡ 35bed5cb-f2f7-4132-84d6-397cbbe1a320
ejercicio18_3()
